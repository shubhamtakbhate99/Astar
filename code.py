...
def dijktras(world,start,goal):
    ...
    cost_to_go=cost_to_come.get(start)+eucledian(start, goal)
    ...
    while open_list:
        ...
        for neighbor,stepcost in neighbors:
          ...
          if in_open_list :
            ...
            # if in_open_list:
            #     if c2c_cost<cost_to_come.get(neighbor):
                   
                  
            #        cost_to_come[neighbor]=c2c_cost
            #        parents[neighbor] = current_node
            #        open_list[idx]=[neighbor,c2c_cost]
            # else:
            #     cost_to_come[(neighbor)] = c2c_cost
            #     parents[(neighbor)] =current_node
            #     open_list.append([neighbor, c2c_cost])
    if not path_found:
...
for p in path:
      ...
      cv2.imshow('map', rotate)
      cv2.waitKey(150)
cv2.imwrite("Final_solution_set.png", img)      
